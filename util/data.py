...
def load_dataset(path_metadata, path_jdx, idx_smiles, target_substruct):
    ...
    for i in tqdm(range(0, len(metadata))):
        ...
        if get_state_label(metadata[i][3]) != 'gas':
            continue
        # if get_state_label(metadata[i][3]) != 'liquid':
        #     continue
        # if get_state_label(metadata[i][3]) != 'solid':
        #     continue
...
def save_pred_results(file_name, data_ids, targets, preds):
    results = {data_ids[i]: {'target': targets[i], 'pred': preds[i].tolist()} for i in range(0, len(data_ids))}
    with open(file_name, 'w') as f:
        json.dump(results, f)


def save_attns(file_name, data_ids, attns):
    attns = {data_ids[i]: attns[i].tolist() for i in range(0, len(data_ids))}
    with open(file_name, 'w') as f:
        json.dump(attns, f)
